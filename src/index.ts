...
Introspection loop — every layer below records itself as you act (zero extra calls to build it); the tools just read it back:
- WHILE ACTING: the J-space detector screens every decision's geometry and attaches "J-space detector [warning|critical]" lines to plan.evidence — read them before executing. A near-tie readout is a coin flip; aliased candidates mean label-based targeting cannot distinguish two elements (target by element id or inspect_viewport instead); a concept conflict means the winner is obstructed or leaves the site.
- WHEN A CHOICE LOOKS WRONG OR FRAGILE: **explain_last_decision** retells it in plain language — winner, runner-up, the word the choice hangs on, prediction vs outcome, and whether your stated confidence has been trustworthy this session. Before repeating or rephrasing an intent, **run_intent_experiment** races phrasings against the live page (nothing executes) and returns the most robust one; if phrasings disagree on the target, the intent is ambiguous here — quote the exact label.
- MID-RUN CHECKPOINT (every ~50 actions): **get_jspace_map** shows which concept axes carry your choices, recurring dead-weight words, and near-boundary decisions; **get_jspace_detections** adds the hazard log and session trends — an unstable_target trend means the page changed under you: re-observe before repeating any intent. On long traversals also run generate_thought_report with includeStream: false.
- END OF RUN: **generate_thought_report** — your cognition as a typed thinking transcript (observe → believe → decide → act → verify, each decision's J-space why inline), thinking-pattern detection (blind retries, thought loops, dithering, hesitation, unverified streaks), and ONE ranked chain-of-thinking optimization plan merging every engine's advice; apply it top-down. **generate_behavior_report** adds episode scoring, observation economy, and the calibration record (Brier score, per-band reliability, the high-confidence actions that failed anyway).
- NEXT RUN: start by reading the previous session's reports — .splice/thought/ and .splice/jspace/ are written automatically at session close, .splice/behavior/ whenever the report runs. Offline, \`splice report\` (and --jspace / --thought) analyzes them from the CLI.
Learning is continuous: optimize_prompt strips words the J-space map has repeatedly seen match nothing (cross-checked against the current page's labels, always reported in transformations).`;
